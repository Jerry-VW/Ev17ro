...
void main_task(intptr_t unused)
{
    ...
    */
    while(1)
    {
        ...
        if (bt_cmd == 2) /* 障害物検知 */ // stop mode
        { //sonar_alert() == 1 機能しないにする
            forward = turn = 0; /* 障害物を検知したら停止 */
        }else if(bt_cmd == 'w'){ // forward
          forward = 60;
          turn = 0;
        }else if(bt_cmd == 's'){ // backward
          forward = -60;
          turn = 0;
        }else if(bt_cmd == 'a'){ // turn left
          forward = 0;
          turn = -30;
        }else if(bt_cmd == 'd'){ // turn right
          forward = 0;
          turn = 30;
        }
        else if(bt_cmd == 1) // trace mode
        {
            // PID here
            forward = 30; /* 前進命令 */
        ...
        balance_control(
            ...
            (float)motor_ang_r,
            (float)volt,
            (signed char*)&pwm_L,   // out put
            (signed char*)&pwm_R);  // out put
...
{
  uint8_t c;
  char iBuffer[6];// 16bit binary -> 0-65535　5 digits,and 1 for NULL
  ...
  while(1)
  {
    ...
    tslp_tsk(10); /* 10msec周期起動 */
    // get到的数据是int16，但serial是依据ascii，十进制sensor返回32，解释为0x20，被表示为空格
  }
}
